...
fun SearchScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            Text(
                text = if (query.isBlank()) "문서 ${documentCount}개 인덱싱됨"
                       else "문서 ${documentCount}개 중 ${hits.size}개 결과",
...
private val previewHits = listOf(
    SearchHit(Document(1, "벚꽃 산책", "벚꽃이 만개한 길을 걸었다. 꽃잎이 바람에 흩날려 마치 눈처럼 떨어졌다."), score = 0f),
    SearchHit(Document(2, "Morning Walk", "Woke up early and took a quiet walk through the park. The fresh air was refreshing."), score = 0f),
...
private fun SearchScreenResultsPreview() {
    HanftsTheme {
        SearchScreen(
            query         = "산책",
            hits          = listOf(
                SearchHit(Document(1, "벚꽃 산책", "벚꽃이 만개한 길을 걸었다. 꽃잎이 바람에 흩날려 마치 눈처럼 떨어졌다."), score = 1.82f),
                SearchHit(Document(2, "한강 산책", "한강 공원을 따라 한 시간쯤 걸었다. 강바람이 시원하고 경치가 좋았다."), score = 1.21f),
            ),
            documentCount = 20,
            onQueryChange = {},
        )
